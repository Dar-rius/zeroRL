...
class TestBufferIntegration:
    ...
    def test_full_insert_cycle_with_get_all(self, device) -> None:
        buf = Buffer(capacity=20, num_envs=1, schema={"state": (8,), "action": (4,)}, device=device)
        for _ in range(20):
    ...
    def test_buffer_scalar_actions(self, device) -> None:
        buf = Buffer(capacity=10, num_envs=1, schema={"state": (4,), "action": ()}, device=device)
        for i in range(10):
    ...
    def test_clear_and_refill(self, device) -> None:
        buf = Buffer(capacity=5, num_envs=1, schema={"state": (2,)}, device=device)
        for i in range(5):
    ...
    def test_large_buffer(self, device) -> None:
        n = 2048
        buf = Buffer(capacity=n, num_envs=1, schema={"state": (4,), "action": ()}, device=device)
        for _ in range(n):
